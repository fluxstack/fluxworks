...
func (o *orchestrator) receiveMessage(message *message.Message) (err error) {
	...
	replyMsg := NewReply(reply, message.Metadata)
	ctx := message.Context()
...
func GetRequired(metadata message.Metadata, key string) (string, error) {
	v, exists := metadata[key]
	if !exists {
...
func (o *orchestrator) replyMessageInfo(message *message.Message) (string, string, string, error) {
	...
	replyName, err = GetRequired(message.Metadata, MessageReplyName)
	if err != nil {
		o.logger.Error("error reading reply name", err, watermill.LogFields{})
		return "", "", "", err
	}

	sagaID, err = GetRequired(message.Metadata, MessageReplySagaID)
	if err != nil {
		o.logger.Error("error reading saga id", err, watermill.LogFields{})
		return "", "", "", err
	}

	sagaName, err = GetRequired(message.Metadata, MessageReplySagaName)
	if err != nil {
...
func (o *orchestrator) processResults(ctx context.Context, instance *Instance, results *stepResults) (err error) {
	...
	for {
		if results.failure != nil {
			...
			results, err = o.handleReply(ctx, results.updatedStepContext, results.updatedSagaData, WithFailure())
			if err != nil {
			...
			for _, command := range results.commands {
				...
				WithHeaders(map[string]string{
					MessageCommandName:         commandName,
					MessageCommandReplyChannel: o.definition.ReplyChannel(),
				})(msg)
			...
			results, err = o.handleReply(ctx, results.updatedStepContext, results.updatedSagaData, WithSuccess())
			if err != nil {
...
func (o *orchestrator) handleReply(ctx context.Context, stepCtx stepContext, sagaData SagaData, message ReplyMessage) (*stepResults, error) {
	...
	outcome, err := GetRequired(message.Headers(), MessageReplyOutcome)
	if err != nil {
	...
	success := outcome == ReplyOutcomeSuccess
...
type MessageOption func(m *message.Message)

// WithHeaders is an option to set additional headers onto the Message
func WithHeaders(headers message.Metadata) MessageOption {
